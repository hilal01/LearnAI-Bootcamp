...
From the portal go into Botlog SQL DATABASE
Top right click on "Connect with" and specify Visual Studio
Follow the steps to connect

2.4.   Create a new table called userChatLog with the below create table statement (or schema). You can run the below query in the Query Editor by selecting Tools -> SQL Server -> New Query.
